...
void Monster::lootMessage(Container *corpse, Player *looter) {
	std::stringstream ss;
	std::string name = this->getDescription(true);
	std::string lootlist = corpse->containerContents(corpse, NULL);
	ss << "Loot of " << name.substr(0, name.find(".")) << ": ";
	ss << lootlist.substr(0, lootlist.length() - 2) << ".";
	looter->sendTextMessage(MSG_INFO, ss.str().c_str());
}
