...
bool IOPlayerXML::loadPlayer(Player* player, std::string name){
	...
	if (doc)
	{
		...
		nodeValue = (char*)xmlGetProp(root, (const xmlChar *) "dead");
		if (nodeValue) {
			player->dead = (atoi(nodeValue) != 0);
			xmlFreeOTSERV(nodeValue);
		}
...
bool IOPlayerXML::savePlayer(Player* player){
	...
	sb << player->dead;  xmlSetProp(root, (const xmlChar*) "dead", (const xmlChar*)sb.str().c_str()); sb.str("");
#endif //TLM_SKULLS_PARTY
