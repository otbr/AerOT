...
std::string Container::containerContents(Container *cur, Container* parent) {
	ContainerList::const_iterator it;
	std::stringstream ss;

	for (it = cur->getItems(); it != cur->getEnd(); ++it) {
		Container *container = dynamic_cast<Container*>(*it);
		if (container)
			ss << container->containerContents(container, parent);
		else {
			ss << (*it)->getName() + ", ";
		}
	}
