...
bool Commands::setSpeed(Creature* c, const std::string &cmd, const std::string &param) {

	Creature* creature;
	Player* player;
	Player* thrower = dynamic_cast<Player*>(c);
	int speed;
	std::stringstream tmp;


	std::string name = param.substr(0, param.find(","));
	creature = game->getCreatureByName(name);
	player 	= game->getPlayerByName(name);

	if (param.find(",") == std::string::npos) {
		thrower->sendTextMessage(MSG_RED_TEXT, "Usage: /speed [NAME], [SPEED]");
		return false;
	}
	speed 	= atoi(param.substr(param.find(",") + 1, param.length() - 1).c_str());
	if (speed < 50)
		speed = 50;

	tmp << "Your speed was set to " << speed;
	std::cout << "Player" << name << "'s speed was set to " << speed;
	creature->speed = speed;
	player->sendTextMessage(MSG_BLUE_TEXT, tmp.str().c_str());
...
bool Commands::unkill(Creature* c, const std::string &cmd, const std::string &param)
{
